...
contract('SideBridge', function(accounts) {
  ...
  it("should be able to approve others to spend tokens in their name", function() {
    ...
    return SideBridge.new(requiredSignatures, authorities, estimatedGasCostOfWithdraw).then(function(instance) {
      ...
      return contract.deposit(owner, 3000000000000000000, hash, {from: authorities[0]});
    }).then(function(result) {
      ...
      return contract.transferFrom(owner, receiver, 1000000000000000000, {from: spender})
        .then(function() {
      ...
      return contract.approve(spender, 4000000000000000000, {from: owner});
    }).then(function(result) {
      assert.equal(1, result.logs.length, "Exactly one event should be created");
      ...
      assert.equal(4000000000000000000, result.logs[0].args.tokens);

      return contract.allowance(owner, spender);
    }).then(function(result) {
      assert.equal(4000000000000000000, result, "approval should set allowance");

      return contract.transferFrom(owner, receiver, 4000000000000000000, {from: spender})
        .then(function() {
          assert(false, "transferring more than balance should fail");
        }, helpers.ignoreExpectedError)
    }).then(function() {

      return contract.approve(spender, 2000000000000000000, {from: owner});
    }).then(function(result) {
      assert.equal(1, result.logs.length, "Exactly one event should be created");
      assert.equal("Approval", result.logs[0].event, "Event name should be Approval");
      assert.equal(owner, result.logs[0].args.tokenOwner);
      assert.equal(spender, result.logs[0].args.spender);
      assert.equal(2000000000000000000, result.logs[0].args.tokens);

      return contract.allowance(owner, spender);
    }).then(function() {

      return contract.transferFrom(owner, receiver, 2000000000000000000, {from: spender});
    }).then(function(result) {
      assert.equal(1, result.logs.length, "Exactly one event should be created");
      assert.equal("Transfer", result.logs[0].event, "Event name should be Transfer");
      assert.equal(owner, result.logs[0].args.from);
      ...
      assert.equal(1000000000000000000, result, "transferring should reduce owners balance");

      return contract.balanceOf(receiver);
    }).then(function(result) {
      assert.equal(2000000000000000000, result, "transferring should increase receivers balance");
  ...
  it("should allow user to transfer value locally", function() {
    ...
    var user1InitialValue = 3000000000000000000;
    var transferedValue = 1000000000000000000;
    var hash = "0xe55bb43c36cdf79e23b4adc149cdded921f0d482e613c50c6540977c213bc408";
    return SideBridge.new(requiredSignatures, authorities, estimatedGasCostOfWithdraw).then(function(instance) {
      meta = instance;
      ...
      assert.equal(2000000000000000000, result[0]);
  ...
  it("should not allow user to transfer value they don't have", function() {
    ...
    var userValue = 3000000000000000000;
    var transferedValue = 4000000000000000000;
    var hash = "0xe55bb43c36cdf79e23b4adc149cdded921f0d482e613c50c6540977c213bc408";
    return SideBridge.new(requiredSignatures, authorities, estimatedGasCostOfWithdraw).then(function(instance) {
      meta = instance;
